...
iv.raw.data <- fread("vxxledailyprices.csv")
...
data0 <- fread("XLE-2010-2011.csv")
data1 <- fread("XLE-2012.csv")
data2 <- fread("XLE-2013.csv")
data3 <- fread("XLE-2014.csv")
data4 <- fread("XLE-2015-Jan-Oct.csv")
...
combined.raw.data <- rbind(data0, data1, data2, data3, data4)
...
# Rename columns
names(combined.raw.data) <- c("symbol", "date", "price", "option", "expiration", "strike", "call.put", "ask",
                              "bid", "mid.price", "iv", "volume", "open.interest", "delta", "vega", "gamma", 
                              "theta", "rho")
...
closed.dates <- c("2010-01-01", "2010-01-18","2010-02-15", "2010-04-02", "2010-05-31",
                  "2010-07-05", "2010-09-06", "2010-11-25", "2010-12-24", "2011-01-17",
                  "2011-02-21", "2011-04-22", "2011-05-30", "2011-07-04", "2011-09-05",
                  "2011-11-24", "2011-12-26", "2012-01-02", "2012-01-16", "2012-02-20",
                  "2012-04-06", "2012-05-28", "2012-07-04", "2012-09-03", "2012-11-22",
                  ...
                  "2014-07-04", "2014-09-11", "2014-11-27", "2014-12-25", "2015-01-19",
...
combined.raw.data <- mutate(combined.raw.data, exp.day = weekdays(expiration, abbreviate = FALSE))
combined.raw.data <- mutate(combined.raw.data, expiration = ifelse(exp.day == "Saturday", expiration - 1,
                                         expiration))
# Previous command converts expiration to an integer
combined.raw.data$expiration  <- as.Date(combined.raw.data$expiration, origin = "1970-01-01")
...
combined.raw.data <- mutate(combined.raw.data, expiration = ifelse(expiration == unique(bad.exp.dates$expiration), 
...
# Rename columns
names(complete.data) <- c("date", "symbol", "price", "option", "expiration", "strike", "call.put", "ask",
                              "bid", "mid.price", "iv", "volume", "open.interest", "delta", "vega", "gamma", 
...
complete.data <- mutate(complete.data, expiration = as.Date(expiration, origin = "1970-01-01"))
complete.data <- mutate(complete.data, exp_type = ifelse(is.element(expiration, exp_fri), "Monthly", "Weekly"))
